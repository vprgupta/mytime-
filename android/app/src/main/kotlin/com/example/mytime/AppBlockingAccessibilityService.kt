            ...
            if (MainActivity.isCommitmentActive) {
                ...
                if (isSettings) {
                    ...
                    if (isMyTimeScreen) {
                        ...
                        // 1.5 Check for Device Admin Deactivation
                        // If we are in the Device Admin screen for MyTime, the text "Deactivate" or "Remove" might be present.
                        val hasDeactivate = combinedText.contains("deactivate") || hasTextOnScreen(rootInActiveWindow, "deactivate")
                        val hasRemoveAdmin = combinedText.contains("remove admin") || hasTextOnScreen(rootInActiveWindow, "remove admin")

                        if (hasDeactivate || hasRemoveAdmin) {
                             android.util.Log.d("AccessibilityService", "🛡️ Commitment Mode: Prevented Admin Tampering (Aggressive)")
                             triggerGlobalActionHome(true)
                             return
                        }
                        ...
                        if (!isSwitchAction) {
                             ...
                             // BLANKET BLOCK: If we are on the "Use MyTime" screen, block it entirely.
                             val isAccessibilityDetails = hasTextOnScreen(rootInActiveWindow, "use service") ||
                                                          hasTextOnScreen(rootInActiveWindow, "use mytime") ||
                                                          hasTextOnScreen(rootInActiveWindow, "stop mytime")

                             if (isAccessibilityDetails) {
